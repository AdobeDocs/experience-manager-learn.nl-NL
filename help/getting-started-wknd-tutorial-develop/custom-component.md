...
U kunt de gebeëindigde code op [GitHub](https://github.com/adobe/aem-guides-wknd/tree/custom-component/solution) altijd bekijken of de code plaatselijk controleren door aan de tak `custom-component/solution` te schakelen.
...
## Wat u {#byline-component} wilt maken
...
## Byline-component {#create-byline-component} maken
...
### Component Node {#create-component-node} maken
...
1. Voeg de volgende eigenschappen aan de knoop `cq:Component` van de component Byline toe.
   ...
   Dit resulteert in `.content.xml` XML:
...
1. Voeg onder het knooppunt `byline` een nieuw bestand `byline.html` toe, dat verantwoordelijk is voor de HTML-presentatie van de component. De naam van het dossier het zelfde als `cq:Component` knoop is belangrijk aangezien het het standaardmanuscript Sling wordt zal gebruiken om dit middeltype terug te geven.
...
1. Onder de `byline` componentnode maakt u een nieuw knooppunt met de naam `cq:dialog` van het type `nt:unstructured`.
1. Werk `cq:dialog` met volgende XML bij. Het is het gemakkelijkst om `.content.xml` te openen en het volgende XML in het te kopiëren/te kleven.
   ...
   Deze knoopdefinities gebruiken [Sling Resource Merger](https://sling.apache.org/documentation/bundles/resource-merger.html) om te controleren welke dialooglusjes van de `sling:resourceSuperType` component, in dit geval **de component van het Beeld van de Componenten van de Kern** worden geërft.
...
1. Onder de `byline` `cq:Component` knoop, creeer een nieuwe knoop genoemd `cq:design_dialog` van type `nt:unstructured`.
1. Werk `cq:design_dialog` met volgende XML bij. Het is het eenvoudigst om `.content.xml` te openen en de onderstaande XML erin te kopiëren/plakken.
...
### De code {#deploy-the-code} implementeren
...
### De component toevoegen aan een pagina {#add-the-component-to-a-page}
...
Omdat wij [de component Byline aan **WKND.Content** Component Group](#create-component-node) via het `/apps/wknd/components/content/byline@componentGroup=WKND.Content` bezit toevoegden, is het automatisch beschikbaar aan om het even welke **Lay-outcontainer** waarvan **Beleid** de **WKND.Content** component toestaat groep, die de container voor de layout van de artikelpagina is.
...
1. Nadat u het dialoogvenster hebt opgeslagen, navigeert u naar [CRXDE Lite](http://localhost:4502/crx/de/index.jsp#/content/wknd/language-masters/en/magazine/guide-la-skateparks/jcr:content/root/responsivegrid/responsivegrid/byline) en controleert u hoe de inhoud van de component is opgeslagen op het knooppunt met inhoud van de bylinecomponent onder de AEM pagina.

   Zoek het inhoudknooppunt van de component Byline onder het knooppunt `jcr:content/root/responsivegrid/responsivegrid`, d.w.z `/content/wknd/language-masters/en/magazine/guide-la-skateparks/jcr:content/root/responsivegrid/responsivegrid/byline`.
...
## Byline Sling Model {#create-sling-model} maken
...
### Geweven afhankelijkheden controleren {#maven-dependency}

Het Byline Sling-model is gebaseerd op verschillende Java API&#39;s die door AEM worden geleverd. Deze API&#39;s worden beschikbaar gesteld via de `dependencies` in het POM-bestand van de module `core`.

1. Open het `pom.xml` bestand onder `<src>/aem-guides-wknd/core/pom.xml`.
1. Zoek de afhankelijkheid van `uber-jar` in de sectie Afhankelijkheden van het pombestand:
   ...
   De [uber-jar](https://docs.adobe.com/content/help/en/experience-manager-65/developing/devtools/ht-projects-maven.html#experience-manager-api-dependencies) bevat alle openbare Java API&#39;s die door AEM worden weergegeven. U ziet dat er geen versie is opgegeven in het bestand `core/pom.xml`. De versie wordt in plaats daarvan gehandhaafd in de Parent-reactorpom die zich aan de basis van het project `aem-guides-wknd/pom.xml` bevindt.
...
1. Zoek de afhankelijkheid voor `core.wcm.components.core`:
   ...
   Dit zijn alle openbare API&#39;s van Java die worden weergegeven door AEM Core Components. AEM Core Components is een project dat buiten AEM wordt onderhouden en heeft daarom een aparte releasecyclus. Daarom is het een afhankelijkheid die afzonderlijk moet worden opgenomen en **niet** inbegrepen met uber-jar is.
...
1. Maak in de module `aem-guides-wknd.core` onder `src/main/java,` een nieuwe Java-interface met de naam `Byline.java` door met de rechtermuisknop op het `com.adobe.aem.guides.wknd.core.models` **pakket > Nieuw > Interface** te klikken. Ga **Byline** als interfacenaam in, en klik Afwerking.
...
### Byline-implementatie {#byline-implementation}
...
1. Maak in de `core`-module onder `src/main/java` een nieuw klassebestand met de naam **BylineImpl.java** door met de rechtermuisknop op het `com.adobe.aem.guides.wknd.core.models.impl`-pakket te klikken en **Nieuw > Class** te selecteren.

   Voor de naam voert u **BylineImpl** in. Voeg **Byline interface** als implementerende interface toe.

   ![byline-implementatie maken](assets/custom-component/create-byline-impl.png)

1. Open `BylineImpl.java`. Het is auto-bevolkt met alle methodes die in de interface `Byline.java` worden bepaald. Voeg de annotaties van het Sling Model toe door `BylineImpl.java` met de volgende klasse-vlakke annotaties bij te werken. Deze `@Model(..)`annotatie is wat van de klasse een Sling Model verandert.
...
#### Het &quot;Afbeeldingsprobleem&quot;aanpakken {#tackling-the-image-problem}
...
1. Controleer of de JCR-eigenschap `fileReference` wordt omgezet in een element.
1. Converteer deze bron naar een model voor het splitsen van kerncomponentafbeeldingen en zorg ervoor dat de methode `getSrc()` niet leeg is.
...
Open `/apps/wknd/components/content/byline/byline.html` in de module `ui.apps` die we hebben gemaakt in de eerdere set-up van de AEM Component.
...
#### Bezig met instantiëren van objecten in HTML {#instantiating-sling-model-objects-in-htl}
...
#### Bezig met benaderen van methoden voor verkoopmodel {#accessing-sling-model-methods}
...
#### De tijdelijke aanduiding {#conditionally-displaying-the-placeholder} voorwaardelijk weergeven
...
3. Implementeer de basis van de code naar een lokale AEM-instantie. Aangezien de belangrijkste veranderingen in POM dossiers werden aangebracht, voer een volledige Gemaakt bouwstijl van de wortelfolder van het project uit.
   ...
   >[!WARNING]
   >
   > Merk op dat het WKND project opstelling dusdanig is dat `ui.content` om het even welke veranderingen in JCR zal beschrijven, daarom willen wij ervoor zorgen wij slechts het `ui.apps` project opstellen om te vermijden het wegvagen van de component Byline die aan de artikelpagina vroeger wordt toegevoegd.
...
#### De naamloze Byline-component {#reviewing-the-unstyled-byline-component} bekijken

1. Nadat u de update hebt geïmplementeerd, navigeert u naar de [Ultimate Guide naar de pagina LA Skateparks ](http://localhost:4502/editor.html/content/wknd/language-masters/en/magazine/guide-la-skateparks.html) of waar u de Byline-component eerder in het hoofdstuk hebt toegevoegd.
...
## Naamregelstijlen {#byline-styles}
...
Standaardstijlen toevoegen voor de component Byline. In het **ui.frontend**-project onder `/src/main/webpack/components/content`:

1. Maak een nieuwe map met de naam `byline`.
1. Maak een nieuwe map onder de map `byline` met de naam `scss`.
1. Maak een nieuw bestand onder de map `byline/scss` met de naam `byline.scss`.
1. Maak een nieuwe map onder de map `byline/scss` met de naam `styles`.
1. Maak een nieuw bestand onder de map `byline/scss/styles` met de naam `default.scss`.

   ![verkenner van byline-project](assets/custom-component/byline-style-project-explorer.png)

1. Start door **byline.scss** te vullen om de standaardstijl op te nemen:
...
1. Open het bestand `main.scss` in het **ui.frontend**-project onder `/src/main/webpack/site` en voeg de volgende regel toe in de sectie `/* Components */`:

   ```scss
   @import '../components/content/byline/scss/byline.scss';
   ```

1. De `ui.frontend` module bouwen en compileren met NPM:
...
1. Het `ui.apps`-project bouwen en implementeren, dat het `ui.frontend`-project tijdelijk omvat, in een lokale AEM-instantie die Maven gebruikt:
...
Bekijk de gebeëindigde code op [GitHub](https://github.com/adobe/aem-guides-wknd) of herzie en stel plaatselijk de code bij de schakelaar van de Git `custom-component/solution` op.
...
Als u `src/main/java` bronmap niet ziet in Eclipse, kunt u de mappen toevoegen door met de rechtermuisknop op src te klikken en mappen voor hoofd en java toe te voegen. Nadat u de mappen hebt toegevoegd, wordt het `src/main/java`-pakket weergegeven.
