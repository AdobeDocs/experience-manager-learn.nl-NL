...
description: Met Adobe Cloud Manager kunt u code samenstellen en implementeren om as a Cloud Service AEM. De mislukkingen kunnen tijdens stappen in het bouwstijlproces voorkomen, die actie vereisen om hen op te lossen. Deze gids loopt door het begrip gemeenschappelijke mislukkingen in de plaatsing, en hoe te om hen het best te benaderen.
...
AEM as a Cloud Service omvat automatisch de nieuwste versie van de Componenten van de Kern in elke AEM Versie, die betekent nadat een AEM as a Cloud Service milieu, automatisch of manueel wordt bijgewerkt, de recentste versie van de Componenten van de Kern aan het wordt opgesteld.
...
`com.adobe.cq.wcm.core.components...` pakket(en) met een specifiek versiebereik kon niet worden geïmporteerd door de `core` project.
...
+ __Resolutie:__ Controleer de `aemerror` logt rond de tijd (logboektijd in GMT) van de fout zoals weergegeven in Cloud Manager voor de services voor auteurs en publiceren van AEM.
...
+ __Oorzaak:__ AEM gebruiker van de replicatieservice die wordt gebruikt om inhoudspakketten aan de publicatieservice op te stellen AEM kan niet schrijven aan `/var` op AEM Publish. Dit resulteert in de plaatsing van het inhoudspakket aan de publicatieservice AEM om te ontbreken.
+ __Resolutie:__ De volgende manieren om deze problemen op te lossen worden weergegeven in de volgorde van voorkeur:
   ...
   3. Als de `/var` bronnen alleen vereist zijn voor AEM auteur en redelijkerwijs niet kunnen worden gemodelleerd met [herwijzen](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/deploying/overview.html#repoinit), verplaatst u deze naar een apart inhoudspakket dat alleen op AEM Author is geïnstalleerd door [insluiten](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/developing/aem-project-content-package-structure.html#embeddeds) in de `all` in een runmode-map van AEM-auteur (`<target>/apps/example-packages/content/install.author</target>`).
   4. Verstrek aangewezen ACLs aan `sling-distribution-importer` servicegebruiker, zoals in dit [Adobe KB](https://helpx.adobe.com/in/experience-manager/kb/cm/cloudmanager-deploy-fails-due-to-sling-distribution-aem.html).
