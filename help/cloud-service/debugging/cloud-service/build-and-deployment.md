...
De bouw en de Testende fase van de Eenheid voert een Maven bouwstijl (`mvn clean package`) van het project uit die uit de gevormde tak van het Git van de pijpleiding wordt gecontroleerd.
...
+ Er wordt een geoorloofde afhankelijkheid gebruikt die niet beschikbaar is op [Maven Central](https://search.maven.org/) en de Geweven opslagplaats die de afhankelijkheid bevat, is:
   ...
   + Niet expliciet geregistreerd in het project `pom.xml`. Merk op dat het opnemen van Maven repositories wordt ontmoedigd omdat het de bouwtijden verhoogt.
+ Eenheidstests mislukken vanwege tijdproblemen. Dit kan zich voordoen wanneer eenheidstests timinggevoelig zijn. Een sterke indicator is afhankelijk van `.sleep(..)` de testcode.
...
Het aftasten van de code resulteert in een bouwstijlmislukking als de Kritieke kwetsbaarheid van de Veiligheid in de code bestaat. Minder overtredingen kunnen worden overschreven, maar het wordt aanbevolen dat deze worden gecorrigeerd. Het scannen van code is niet perfect en kan leiden tot [onjuiste positieven](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/developing/understand-test-results.html#dealing-with-false-positives).

Als u problemen met het scannen van code wilt verhelpen, downloadt u het CSV-rapport dat door Cloud Manager via de knop Details **** downloaden wordt aangeboden en controleert u alle items.

Zie voor meer informatie AEM specifieke regels de [aangepaste AEM-specifieke code scanning-regels](https://docs.adobe.com/content/help/en/experience-manager-cloud-manager/using/how-to-use/custom-code-quality-rules.html)voor documenten in Cloud Manager.
...
+ __Resolutie:__ Herzie alle pom.xml- pakketten ingebed in het al project, en zorg ervoor zij de `filevault-package-maven-plugin` configuratie [hebben die aan wordt geplaatst](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/developing/aem-project-content-package-structure.html#cloud-manager-target) `<cloudManagerTarget>none</cloudManagerTarget>`.
...
+ De het opstellen toepassing werkt de Kern Componenten in kaart gebrachte gebiedsdeelversie in het `core` (bundel OSGi) project bij
...
+ __Symptomen:__
De stap Build Image mislukt met een ERROR-melding dat 
`com.adobe.cq.wcm.core.components...` pakket(en) met specifieke versiereiken kon niet door het `core` project worden geïmporteerd.
...
+ __Oorzaak:__  De bundel OSGi van de toepassing (die in het `core` project wordt bepaald) voert de klassen van Java van de kern van Componenten van de Kern gebiedsdeel, op een verschillend versieniveau in dan wat aan AEM als Cloud Service wordt opgesteld.
+ __Resolutie:__
...
Verken me met [AEM als logboeken](./logs.md) van de Cloud Service voorafgaand aan het zuiveren van Deploy aan stap. Het `aemerror` logbestand bevat informatie over het opstarten en afsluiten van pods die relevant kan zijn voor het implementeren van problemen. Het logbestand dat beschikbaar is via de knop Logbestand downloaden in de stap Implementeren van Cloud Manager voor stap is niet het `aemerror` logbestand en bevat geen gedetailleerde informatie over het opstarten van uw toepassingen.
...
+ __Resolutie:__ Controleer de `aemerror` logbestanden voor AEM-auteur- en -publicatieservices rond de tijd (logtijd in GMT) van de fout zoals deze wordt weergegeven in Cloud Manager.
...
`/var` is veranderbaar die een verscheidenheid van voorbijgaande, runtime inhoud bevat. Opnemen `/var` in inhoudspakketten (bijv. `ui.content`) geïmplementeerd via Cloud Manager kan ertoe leiden dat de stap Implementeren mislukt.
...
+ __Oorzaak:__ AEM gebruiker van de replicatieservice die wordt gebruikt om inhoudspakketten op te stellen aan de AEM publicatieservice kan niet schrijven aan `/var` op publiceren AEM. Dit resulteert in de plaatsing van het inhoudspakket aan de publicatieservice AEM om te ontbreken.
+ __Resolutie:__ De volgende manieren om deze problemen op te lossen worden weergegeven in de volgorde van voorkeur:
   ...
   3. Als de `/var` bronnen alleen vereist zijn voor AEM auteur en niet op een redelijke manier kunnen worden gemodelleerd met [opnieuw aanwijzen](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/deploying/overview.html#repoinit), verplaatst u ze naar een apart inhoudspakket dat alleen op de AEM-auteur wordt geïnstalleerd door deze in te [sluiten](https://docs.adobe.com/content/help/en/experience-manager-cloud-service/implementing/developing/aem-project-content-package-structure.html#embeddeds) in het `all` pakket in een runmode-map (`<target>/apps/example-packages/content/install.author</target>`) van AEM-auteur.
