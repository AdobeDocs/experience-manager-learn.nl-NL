...
2. In `Edit` modus, voegt u de `Card` aan de `Layout Container`:
...
7. Een nieuw tabblad openen en naar [CRXDE-Lite](http://localhost:4502/crx/de/index.jsp#/content/wknd-spa-angular/us/en/home/jcr%3Acontent/root/responsivegrid/card). Inspect the content nodes under `/content/wknd-spa-angular/us/en/home/jcr:content/root/responsivegrid` om de `Card` componentinhoud.
   ...
   Deze eigenschappen waarnemen `cardPath`, `ctaText`, `titleFromPage` worden door het dialoogvenster voortgezet.
...
5. Voer vervolgens de `initModel()` methode om een variabele van het type private te initiëren `cardPage` op basis van de waarde van `cardPath`
   ...
   De `@PostConstruct initModel()` wordt aangeroepen wanneer het Sling-model wordt geïnitialiseerd. Het is daarom een goede gelegenheid om objecten te initialiseren die door andere methoden in het model kunnen worden gebruikt. De `pageManager` is één van verscheidene [Door Java™ ondersteunde globale objecten](https://experienceleague.adobe.com/docs/experience-manager-htl/content/global-objects.html) via het `@ScriptVariable` aantekening. De [getPage](https://developer.adobe.com/experience-manager/reference-materials/cloud-service/javadoc/com/day/cq/wcm/api/PageManager.html) de methode neemt in een weg en keert een AEM terug [Pagina](https://developer.adobe.com/experience-manager/reference-materials/cloud-service/javadoc/com/day/cq/wcm/api/Page.html) -object of null als het pad niet naar een geldige pagina verwijst.

   Hiermee initialiseert u de `cardPage` variabele, die door de andere nieuwe methoden wordt gebruikt om gegevens over de onderliggende gekoppelde pagina te retourneren.
