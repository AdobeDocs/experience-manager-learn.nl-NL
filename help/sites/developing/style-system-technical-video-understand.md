...
Het verstrekte Pakket van de AEM (**technisch-review.sites.style-system-1.0.0.zip**) installeert de stijl van de voorbeeldtitel, steekproefbeleid voor de componenten van de Lay-out Wij.Retail en van de Titel, en een steekproefpagina.

[technical-review.sites.style-system-1.0.0.zip](assets/technical-review.sites.style-system-1.0.0.zip)

### De CSS {#the-css}

Hier volgt de [!DNL LESS] definitie van de voorbeeldstijl:

* `/apps/demo/sites/style-system/clientlib-example/components/titles/styles/example.less`

Voor diegenen die liever CSS gebruiken, is onder dit codefragment de CSS die dit codefragment [!DNL LESS] bevat.
...
>[!NOTE]
>
>Het Stijlsysteem maakt een kleine technische afwijking van [BEM](https://en.bem.info/), in die zin `BLOCK` en `BLOCK--MODIFIER` niet op het zelfde element, zoals gespecificeerd door [BEM](https://en.bem.info/)worden toegepast.
>
>Vanwege productbeperkingen `BLOCK--MODIFIER` wordt de code in plaats daarvan toegepast op het bovenliggende element van het `BLOCK` element.
>
>Alle andere huurders van [BEM](https://en.bem.info/) moeten worden uitgelijnd.

* Gebruik preprocessoren zoals [LESS](https://lesscss.org/) (ondersteund door AEM native) of [SCSS](https://sass-lang.com/) (aangepast builsysteem vereist) voor duidelijke CSS-definitie en herbruikbaarheid.
...
* Elke stijl indelen in een afzonderlijk bestand.
   * Deze bestanden kunnen worden gecombineerd met behulp van LESS/SCSS `@imports` of als onbewerkte CSS is vereist, via de insluiting van HTML-clientbibliotheekbestanden of aangepaste front-end systemen voor het maken van elementen.
...
* Vermijd het rechtstreeks opmaken, `BLOCK--MODIFIER` aangezien dit aan het Responsieve Net in bijlage is. Het wijzigen van de weergave van dit element kan van invloed zijn op de rendering en functionaliteit van het responsieve raster. Stijl dus alleen op dit niveau wanneer de intentie bestaat om het gedrag van het responsieve raster te wijzigen.
* Pas stijlbereik toe met `BLOCK--MODIFIER`. De stijl `BLOCK__ELEMENT--MODIFIERS` kan worden gebruikt in de component, maar aangezien de component de component vertegenwoordigt, en de component is wat wordt gestileerd, is de Stijl &quot;bepaald&quot;en scoped via `BLOCK` `BLOCK--MODIFIER`.
...
In het geval van geneste componenten overschrijdt de CSS-selectierdiepte voor deze geneste componentelementen de selector op het derde niveau. Herhaal hetzelfde patroon voor de geneste component, maar het bereik wordt ingesteld door de bovenliggende component `BLOCK`. Met andere woorden, start u de geneste component `BLOCK` op het derde niveau en de geneste component `ELEMENT` bevindt zich op het vierde selectieniveau.
...
* U voorkomt flikkerende en onnodige hertekeningen door eerst de component via te verbergen `BLOCK--MODIFIER BLOCK`en weer te geven wanneer alle DOM-manipulaties in JavaScript zijn voltooid.
